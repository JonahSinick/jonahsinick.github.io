...
export function reconstructPath(result, start, target) {
  ...
  const targetKey = gridKey(target.x, target.z, result.width);
  if (!result.prev.has(targetKey)) return null;

  const path = [];
  let current = [target.x, target.z];
  while (!(current[0] === start.x && current[1] === start.z)) {
    path.push(current);
    current = result.prev.get(gridKey(current[0], current[1], result.width));
    // A malformed or externally reconstructed predecessor map should fail
    // closed instead of turning the remaining target into an illegal jump.
